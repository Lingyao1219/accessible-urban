...
# Read data
park_df <- read.csv('D://Google_Review//Accessible//ct_senti_access.csv')
park_df$STFIPS <- as.factor(park_df$STFIPS)
...
influential <- as.numeric(row.names(park_df)[(park_df$.cooksd > (2 / (nrow(park_df)-10)))]) # default: 2
...
GAM_RES1 <- mgcv::bam(weight_st ~ Asian_R + Black_Non_Hispanic_R + HISPANIC_LATINO_R + Indian_R
        ...
        + ti(Lat, Lng, bs = 'gp') + s(STFIPS, bs = 're'), data = park_df1,
...
# Nonlinear
GAM_RES1 <- mgcv::bam(weight_st ~ s(Asian_R) + s(Black_Non_Hispanic_R) + s(HISPANIC_LATINO_R) + s(Indian_R)
        + s(Population_Density) + s(Employment_Density) + s(Rural_Population_R)
        + s(Household_Below_Poverty_R) + s(Education_Degree_R)
        + s(Male_R) + s(Over_65_R) + s(Bt_45_64_R) + s(Disability)
        + s(Accessible_review_density) + s(avg_poi_score)
      + ti(Lat, Lng, bs = 'gp') + s(CBSA, bs = 're'), data = park_df1,
        control = gam.control(trace = TRUE), method = "fREML", discrete = TRUE)
summary(GAM_RES1)
b <- getViz(GAM_RES1, scheme = 3)
pl <- plot(b, select = 1:15) # , select = 2:7
print(pl, pages = 3)