...
poly['CTFIPS'] = poly['GISJOIN'].str[1:3] + poly['GISJOIN'].str[4:7]
poly['STFIPS'] = poly['GISJOIN'].str[1:3]
poly = poly[~poly['STFIPS'].isin(['02', '15', '60', '66', '69', '72', '78'])].reset_index(drop=True)
# poly = poly.to_crs(epsg=5070)

# Read other features
CT_Features = pd.read_csv(r'F:\Research_Old\COVID19-Socio\Data\County_COVID_19.csv', index_col=0)
...
smart_loc['CTFIPS'] = smart_loc['BGFIPS'].str[0:5]
smart_loc.loc[smart_loc['D4C'] < 0, 'D4C'] = 0
smart_loc.loc[smart_loc['D4D'] < 0, 'D4D'] = 0
smart_loc.loc[smart_loc['D4E'] < 0, 'D4E'] = 0
smart_loc.loc[smart_loc['D4A'] < 0, 'D4A'] = 1207.008
smart_loc = smart_loc.groupby(['CTFIPS'])[
    ['D3B', 'Pct_AO0', 'Pct_AO1', 'Pct_AO2p', 'NatWalkInd', 'D3A', 'D1C', 'D4C', 'D4A', 'D4D', 'D4E']].mean()
...
g_pois['CTFIPS'] = g_pois['BGFIPS'].str[0:5]
...
g_pois['sum_comment'] = g_pois.groupby(['CTFIPS'])['total_accessible_reviews'].transform("sum")
...
# Average sentiment to CT
bg_pois = g_pois.groupby(['CTFIPS'])['weight_st'].sum().reset_index()
bg_count = g_pois.groupby(['CTFIPS'])['total_accessible_reviews'].sum().reset_index()
bg_pois = bg_pois.merge(bg_count, on=['CTFIPS'])
bg_pois = bg_pois[bg_pois['CTFIPS'].isin(poly['CTFIPS'])].reset_index(drop=True)

# Rename
CT_Features = CT_Features.merge(smart_loc, on='CTFIPS')
...
disability = disability[
    ['GISJOIN', 'AQR0E001', 'AQR0E005', 'AQR0E008', 'AQR0E012', 'AQR0E015', 'AQR0E020', 'AQR0E023', 'AQR0E027',
     'AQR0E030']]
disability['CTFIPS'] = disability['GISJOIN'].str[1:3] + disability['GISJOIN'].str[4:7]
disability = disability.groupby('CTFIPS').sum().reset_index()
...
g_fe['CTFIPS'] = g_fe['BGFIPS'].str[0:5]
g_fe_sum = g_fe.groupby(['CTFIPS'])[['num_of_reviews', 'total_poi_count']].sum().reset_index()
g_fe_avg = g_fe.groupby(['CTFIPS'])[['avg_poi_score']].mean().reset_index()
g_fe_all = g_fe_sum.merge(g_fe_avg, on=['CTFIPS'], how='left')
bg_pois = bg_pois.merge(g_fe_all, on='CTFIPS').reset_index(drop=True)
bg_pois_raw = bg_pois.copy()

# Merge with CBG features
bg_pois = bg_pois.merge(CT_Features, on='CTFIPS').reset_index(drop=True)
# Only greater than 10
bg_pois = bg_pois[(bg_pois['total_accessible_reviews'] > 10)].reset_index(drop=True)
bg_pois = bg_pois[~bg_pois['CTFIPS'].isin(outliers_ns)].reset_index(drop=True)
bg_pois = bg_pois.fillna(0)
sns.displot(bg_pois['weight_st'])
plt.xlabel('County sentiment')
plt.savefig(r'D:\Google_Review\Accessible\dis_sentiment.pdf')

# All CT: Merge with CT features
bg_pois_raw = bg_pois_raw.merge(CT_Features, on='CTFIPS', how='right').reset_index(drop=True)
# Polish data
...
# Only greater than 10
# bg_pois = bg_pois[(bg_pois['total_accessible_reviews'] > 10)].reset_index(drop=True)
bg_pois_raw = bg_pois_raw[~bg_pois_raw['CTFIPS'].isin(outliers_ns)].reset_index(drop=True)
...
need_scio = ['Population_Density', 'Bt_18_44_R', 'Asian_R', 'Over_65_R',
             ...
             'Road_Density', 'Disability',
             'Education_Degree_R', 'Median_income', 'Accessible_review_density',
             'num_of_reviews', 'total_poi_count', 'avg_poi_score', 'total_accessible_reviews',
             "Total_review_density", "Total_poi_density", 'Disability_below_poverty_R', 'Disability_above_poverty_R']
fbg_pois = bg_pois[['CTFIPS', 'weight_st', 'ALAND', 'Lng', 'Lat'] + need_scio]
fbg_pois['STFIPS'] = fbg_pois['CTFIPS'].str[0:2]
fbg_pois.to_csv(r'D:\Google_Review\Accessible\ct_senti_access.csv')
fbg_pois.isnull().sum()
fbg_pois.corr(numeric_only=True).to_csv(r'D:\Google_Review\Accessible\ct_access_corr.csv')
fbg_pois.describe().T.to_csv(r'D:\Google_Review\Accessible\ct_access_des.csv')
...
def plot_scatter(poi_msa, x_t, x_name, cck):
    ...
    sns.regplot(x=poi_msa[x_t], y=(poi_msa['weight_st']), color=cck,
                scatter_kws={'s': (poi_msa['size']), 'alpha': 0.3}, ax=ax)
...
bg_pois_geo = poly.merge(bg_pois.loc[:, ['CTFIPS', 'weight_st']], on='CTFIPS')
...
(poly.geometry.boundary.plot(color=None, edgecolor='gray', linewidth=0.1, alpha=0.8, ax=ax))
ax.tick_params(left=False, labelleft=False, bottom=False, labelbottom=False)
ax.axis('off')
plt.tight_layout()
plt.savefig(r'D:\Google_Review\Accessible\spatial_sentiment_ct.pdf')
plt.close()
...
bg_pois_raw['total_accessible_reviews'] = bg_pois_raw['total_accessible_reviews'].fillna(0)
bg_pois_raw.loc[bg_pois_raw['total_accessible_reviews'] < 10, 'is_access'] = False
bg_pois_raw['is_access'].value_counts()
...
plt.tight_layout()
plt.show()
plt.savefig(r'D:\Google_Review\Accessible\diff_ct.pdf')
